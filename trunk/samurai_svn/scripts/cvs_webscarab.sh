...
# CVS repository
CVS=pserver:anonymous@owasp.cvs.sourceforge.net:/cvsroot/owasp
...
if [ $? = 0 ]; then
	echo
	echo "*** You now need to compile $TOOL... and generate webscarab*.jar"
	echo "*** javac (JDK) required but not found on Samurai v0.7"

	# ant is required to build the project: build.xml
	#cd $SAMURAI_DIR/$TOOL
	#ant compile
fi
